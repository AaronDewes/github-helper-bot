...
class CmdLabel extends command_1.default {
    static run(context, args, _isPR) {
        if ((0, helpers_1.labelExists)(context.octokit, context.repo().owner, context.repo().repo, args)) {
            (0, helpers_1.addLabel)(context.octokit, context.repo().owner, context.repo().repo, context.issue().issue_number, args, '');
        }
        else {
