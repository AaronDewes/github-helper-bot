...
async function remind(context, args) {
    const reminder = (0, parse_reminder_1.default)(`remind ${args}`, '');
    if (reminder) {
        if (reminder.who === 'me') {
...
class CmdRemind extends command_1.default {
    static run(context, args, _isPR) {
        remind(context, args);
    }
}
exports.default = CmdRemind;
CmdRemind.helptext = 'Remind someone of something at a specific time. Example: `/remind [who] [what] [when]`. Who can either be "me" or any GitHub user (like @octocat).';
