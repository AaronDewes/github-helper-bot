...
async function helpText(context) {
    ...
    if (context.payload.sender.login == "louneskmt") {
        context.octokit.issues.createComment({ ...context.issue(), body: `@louneskmt @AaronDewes didn't implement this yet. You can tell everyone in the French podcasts about this.\nAnd Pretzel will win in Baguette vs. Pretzel!` });
    }
    context.octokit.issues.createComment({ ...context.issue(), body: `:wave: ${context.payload.sender.login} Thank you for your interest in this bot!\nUnfortunately, there is no help text yet. @AaronDewes should add one, but he didn't do it yet.` });
}
