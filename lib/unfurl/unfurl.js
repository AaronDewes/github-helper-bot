...
async function unfurl(context, html) {
    const links = (0, get_links_1.default)(html);
    if (links.length > 0) {
        const unfurls = await Promise.all(links.map((link) => (0, unfurl_to_attachment_1.default)(link)));
        unfurls.forEach((unfurl) => {
            (0, probot_attachments_1.default)(context).add(unfurl);
        });
    }
}
