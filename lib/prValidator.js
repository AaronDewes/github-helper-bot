...
async function validatePr(context) {
    ...
    if (config.invalidPRConfig?.addLabel) {
        await helpers_1.addLabel(context.octokit, context.repo().owner, context.repo().repo, context.issue().issue_number, config.invalidPRConfig.labelName || config_1.defaultConfig.invalidPRConfig.labelName, config.invalidPRConfig.labelColor || config_1.defaultConfig.invalidPRConfig.labelColor);
    }
